...
> 朋友圈限定社区（约 20 人）-- 带师德墙、AI 问答和 视觉小说 叙事体验的 Web 平台
...
男德学院是一个面向朋友圈的限定社区平台。除了常规的 Web 功能（认证、管理后台、AI 问答、师德墙），还有一个名为「德塔」的核心模块 -- 一个基于世界观设定集（黑深残×戏谑反差，118 年正史）的 视觉小说 叙事引擎，支持立绘对话、分支选项、好感度系统和服务端存档。
...
- 德塔 视觉小说 -- Vue3 自研叙事引擎，立绘对话、打字机效果、选项分支、好感度系统、服务端存档
...
│       ├── components/     # 视觉小说 组件（对话框/立绘/选项/存档/设置等）
│       ├── stores/         # 视觉小说Store（状态 + 存档/进度 API 对接）
│       └── data/           # 剧本数据（prologue.js 等）
├── server/                 # Express API 后端
│   ├── src/
│   │   ├── controllers/    # 控制器（auth/chat/wall/视觉小说/announcement 等）
│   │   ├── routes/         # 路由聚合
