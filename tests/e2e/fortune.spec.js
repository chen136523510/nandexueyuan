...
/**
 * 观星台（FortuneCard）专项测试
 *
 * 覆盖：登录 -> 大厅布局（词云 8 列 + 观星台 4 列）-> 今日运势内容完整性
...
test.describe('观星台运势模块', () => {
  ...
  test('布局：词云 8 列 + 观星台 4 列（桌面）；1023px 以下全宽', async ({ page }) => {
    // 桌面：两卡同排
  ...
  test('深色模式：观星台卡片无回归（背景/文字 token 生效）', async ({ page }) => {
