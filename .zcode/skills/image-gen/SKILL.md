...
> ⚠️ **tag 堆叠已全面过时**（2025-2026 调研结论，详见 `prd/01-需求文档/00-调研/01-技术/提示词工程调研-图片与视频生成.md`）。新一代模型文本编码器从 CLIP 升级为 T5-XXL / 大语言模型，训练数据是自然语言描述而非 Danbooru 标签。`masterpiece, best quality, 8k` 等质量修饰词在语义模型中是无意义噪声，不仅无效反而有害。
...
当需要生成视频 CG（如国会事变演出）时，提示词写法与图片不同，详见 `prd/01-需求文档/00-调研/01-技术/提示词工程调研-图片与视频生成.md` 第三章。核心要点：
