...
## 2026-09-24 v4.1.0 发版：自习室直播间化（公共消息流 + 实时广播）

### 概要

院长体验 v4.0.0 后两点修正：①自习室入口需出现在顶部导航；②"不需要新的闲聊之类的，做成类似直播间的文字互动"。本次将自习室从"每人独立会话（类男德通私聊）"重构为**直播间形态**：所有人共享同一条公共消息流（nono_messages 表，userId null=诺诺发言），诺诺在流里回复——这回到了 R-058 最初的产品定义（全站共享一只诺诺，所有人可见）。

### 决策依据

- **公共流而非私聊**：直播间感 = 大家看到彼此和诺诺的全部对话，诺诺"在场"于公共空间。L2 记忆从会话摘要改为"最近 30 条公共流"；L1/L3 记忆仍按触发者检索与固化（她认人，在公共流里区分谁在说话）。
- **EventSource 广播而非轮询/ WebSocket**：GET /api/nono/stream（token 走 query，auth 中间件已有支持），进程内订阅者 Set 广播 user_message/nono_typing/nono_token/nono_message 四类事件+25s 心跳——所有在线者实时看到诺诺逐字说话，比 Colyseus 方案轻一个量级（PM2 单进程内存共享成立）。
- **串行回复队列 + 天然攒批**：诺诺一次只回一轮；回复期间新到的消息不各自触发，回完检查 lastRepliedMsgId 之后是否有未覆盖消息再接一轮——多人同时说话时她"听完再答"，直播感且防 LLM 并发爆。
- **NonoMessage 不建 Prisma 关联**：外键列已有，include user 需要模型关联定义（会引出 FK 迁移），改批量查昵称（withNicknames helper）零迁移风险。

### 实测记录

curl 全事件序列（connected → user_message → nono_typing → 20×nono_token → nono_message 含 memoriesSaved）+ Playwright 页面级（历史加载/TopBar 入口/广播到达/流式气泡/✦记住了/座位区说话动效）全部通过。修复一枚：include user 缺关联 500。

### 影响评估

- 原 sessions 三接口删除（前端不再引用）；旧 [自习室] 前缀 ChatSession 数据留库无害。
- 记忆面板保留（开发阶段可视化，开放后收敛后台——院长同日裁决红线）。
- 直播流历史翻页（before 游标）接口已备，前端向上翻页加载留待下轮。
