...
## 2026-07-23 德塔战斗/装备/怪物系统 + 游戏性设计调研方案（V1）

### 决策依据
- **背景**：德塔 MVP 功能已全部上线（P0-P5 + 师德墙），但游戏性偏轻——成员进入德塔主要是聊天和看风景，缺少"每天想回来"的留存机制。用户提出需求：调研战斗系统、装备系统、怪物系统，设计让成员愿意留在德塔的游戏性
- **核心前提**（用户确认）：
  1. 社交驱动：留存靠共同回忆和人情味，不靠排行榜
  2. 纯 PVE：成员之间不互相攻击，只合作打怪物
  3. 20 人小圈子：不套用 MMO 副本/公会/拍卖行
- **方案选择**：完整调研方案，含路径对比 + 推荐方案 + 技术可行性 + 分阶段实施建议

### 四系统推荐方案摘要

| 系统 | 推荐方案 | 核心理念 |
|------|---------|---------|
| 战斗 | **塔楼楼层副本**（路径 B）：二层/三层作为战斗区，爬梯进入切换 | 战斗区隔离，大厅保持社交；楼层闲置已渲染，改造量最小 |
| 装备 | **社区共建装备库**（B+C 混合）：打怪掉材料（公共），全社区合成装备（皮肤），展示在武器架 | 装备是"我们的"不是"我的"，零数值压力 |
| 怪物 | **每日世界BOSS + 每周主题怪**（B+C 混合）：晚20-22点3-5人合作击杀；周日轮换新弱点 | 强社交事件感 + 每周新鲜感 |
| 游戏性 | **三大留存支柱**：每日仪式感（BOSS/签到）、共同目标（共享装备/塔楼建设）、人情记忆（战绩墙/战利品陈列） | 不重但持续，熟人圈靠共同回忆留存 |

### 关键设计
1. **不做 PVP/数值成长/经济系统**：20 人圈不适合，违背社交驱动
2. **等身幽默感**：怪物命名借鉴男德学院调性（"卷王"、"Deadline 怪"、"摸鱼精"）
3. **分阶段实施**：阶段1（MVP 战斗）2-3周 → 阶段2（装备+扩展）2-3周 → 阶段3（周常+建设）3-4周
4. **技术难度**：战斗中/装备中/怪物中高/游戏性低中
5. **可验证假设**：阶段1 验证"20人是否愿意每天上线打BOSS"

### 影响评估
- **新增文件**：`prd/01-需求文档/04-德塔/01-需求/德塔战斗系统调研方案.md`（产品调研文档，约 800 行）
- **数据库预估新增表**：Monster/MonsterSpawn/Material/Equipment/EquipmentLog/BattleRecord/CheckIn/TowerProgress（8 张，待 PRD 阶段细化）
- **关联文档**：`德塔世界观.md` V2 扩展路线、`ADR-003`、`MVP需求文档`
