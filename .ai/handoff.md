...
> 最后更新：2026-08-03 23:22（白机：第一章编剧评审+美术资产清理+班形象设计/脸模+SKILL流程补充）
> 所在设备：白机（荣耀便携本）
> 稳定版本：v2.4.0 已部署上线（commit `811410d`，代码产出 commit `53ab43f`+`fef6e7f`）
> **当前阶段**：M-G1 引擎核心 PoC ✅ + 序章四幕全量落地 ✅ + 存档系统 ✅ + Seedream调研 ✅ + 10角色脸模入库 ✅ + 序章6张背景图接入 ✅ + 见/幸/添立绘8张+表情差分 ✅ + 立绘舞台状态机制 ✅ + 立绘rembg重抠8/8 ✅ + 立绘位置规则(小腿中部对齐) ✅ + 序章结束场景热点交互 ✅ + 部署上线 ✅ + 存档系统优化(自动恢复+新建从头) ✅ + 热点添对话Q&A问答+主线隔离 ✅ + 世界格局地图生成+地图面板交互 ✅ + 序章旁白优化(四幕44->42句)+pro_302穿帮修正 ✅ + 图片预加载机制(首次加载+进度条) ✅ + 世界书评审+补充11条目 ✅ + 设定集v1.4过时内容修正 ✅ + 第一章大纲产出 ✅ + 社区站视觉体系统一 ✅ + **第一章编剧评审(5条改进建议待裁决) ✅ + 美术资产清理(旧版归档+rembg同步) ✅ + 班形象设计定稿+脸模入库 ✅ + image-gen SKILL生成分层流程补充 ✅**

---

## 白机本轮产出（2026-08-03 23:22）

### 第一章编剧评审 + 美术资产清理 + 班形象/脸模（本轮）

**1. 酒馆「德塔编剧」第一章大纲 v1.2 评审**
- 启动酒馆（本地 8000），新聊天上下文，编剧对第一章大纲 5 维度评审通过
- 5 条改进建议（待院长裁决）：
  - ①合同选择前缺北方局势模糊感知（第二幕加添嘀咕"北边信好久没来"）
  - ②商人丙信息太完整（拆碎到三个商人，由添说出"丘"落地）
  - ③帝桥选择①B/C无跨幕回响（第二幕幸来访按选择给带钩子回应）
  - ④村民"屏障"术语化（改生活化"海上的墙矮了/漏了"）
  - ⑤海盗线选择③四选项后果未锚定（改在"关系/资产"上做差异）

**2. 美术资产清理**
- md5 校验发现美术资产立绘全部是旧版（jimp抠图），public/ 才是 rembg 线上版
- 旧版 10 张归档至 `.ai/_backup_portraits/美术资产旧版_20260803/`（已 gitignore）
- 用 public/ 最新版同步替换，md5 全部一致

**3. image-gen SKILL.md 补充生成分层流程**
- 新增「纪律 1.5：生成分层流程」——形象设计→脸模→立绘三层递进
- 强制工作流改为两阶段（出脸模/出立绘），补 rembg抠图/服饰院长过目/带背景原图留存等红线
- 辨识度矩阵新增班条目+黑发系四人撞色提示

**4. 班形象设计定稿+脸模入库**
- 新建 `美术资产/班/班-形象设计.md`：坚毅果敢/对内温柔调皮对外冷酷狠辣/中分黑发黑瞳/背部虚空爪痕/磨牙石吊坠信物/两套服装（日常送葬人装+外出牛仔装）
- 脸模 `face_v1_01.png` 入库（Seedream Pro，v1_02版选中，嘴角玩世不恭smirk+锐利眼神）
- 踩坑：Seedream API image 参数需 data URI 格式（BUG-52）

### 待办（下一步）

- [ ] 院长裁决第一章编剧 5 条改进建议 -> 逐幕打磨台词
- [ ] 班两套立绘出图（日常装「送葬人」基准+外出装「牛仔」，基于刚入库脸模）
- [ ] 班形象确认后回填世界书 5.10 条目（形象待补->定稿）
- [ ] 科夫/西娜形象设计（第一章另两个新 NPC）
