...
> 最后更新：2026-08-21 14:15（白机：**男德通 AI 优化两批已部署上线**。FTS5 方案A 部署落地：① 服务器 .env VOLC_MODEL glm-5.2->glm-5.3 ② prod.db `ALTER TABLE chat_sessions ADD COLUMN summary TEXT`（记忆压缩列） ③ `node scripts/rebuildFtsV2.js` 重建 FTS5 v2 索引（30 秒完成，5,372 分块 + 538,915 消息，纯本地 SQLite+JS CPU 操作不调 LLM）④ npm install+build+pm2 restart。**线上验证全指标通过**：API 200 / 前端 HTTPS 200 / 2 字词「考研」命中 202 个话题块（旧 trigram 版 0 命中）/「打球」命中 23 块 / summary 列就位 / glm-5.3 生效 / 端到端 SSE 提问"群里聊过考研吗"20.6s 出结构化回答（同义词扩展 11 词->FTS5 v2 命中 5 块 505 条->glm-5.3 normal 人设表格+要点输出）。两批 commit `2a1304a`(人设重构/glm-5.3/闲聊上下文)+`6717042`(13 项痛点)+`8d07a91`(文档同步)。服务器 IP 以 `docs/account-passwords.md` 为准：47.96.158.104）
