...
> 最后更新：2026-08-24 09:25（白机：**主模型切换 deepseek-v4-flash**。院长指示算力紧张，glm-5.3 -> `deepseek-v4-flash-ga-260731`。llm.js MODEL/超时(180s→60s)改 + planner/feedback 传 thinking:disabled 省算力提速(实测 planner 1.1-1.6s)。本地验证 planner 3/3 + e2e 3/3 + 冒烟通过。**线上服务器 .env 未动**，需 SSH 后改 VOLC_MODEL 或随下次 deploy 一起。**黑机 8-23 遗留部署待办仍需执行**：deploy.sh + add-nickname-index.js + npm install 补 @dagrejs/dagre）
> 所在设备：白机（判定依据：周一 09:02 工作日白天时段）
> 稳定版本：**v3.6.0 线上**（男德通 AI 优化：glm-5.3 + FTS5 v2 + 记忆压缩 + 人设重构 + 多轮追问；本地已切 deepseek-v4-flash 待部署）
...
## 白机本轮产出（2026-08-24 09:25）

### 男德通主模型切换 deepseek-v4-flash（算力紧张降级）
...
- **验证**：planner JSON 3/3（`verify-deepseek-planner.mjs`）+ e2e 3/3（`verify-deepseek-e2e.mjs`，含流式首 token 3.8s）+ 冒烟 testLlm.js 200
- **⚠️ 线上未部署**：服务器 .env 的 VOLC_MODEL 还是 glm-5.3，下次 SSH 部署时必须手动改（deploy.sh 不覆盖 .env）。部署后观察 deepseek 回答风格差异（更直接少寒暄），群友反馈再调人设
- **⚠️ 部署顺序提醒**：黑机 8-23 的 R-049 全量分析 + BUG-74/75 + 检索优化也未部署，两者一起部署时按黑机待办清单执行（索引脚本 + npm install + deploy.sh + 线上实测全量分析）
