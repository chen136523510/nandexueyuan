...
> 最后更新：2026-08-21 12:05（白机：**岁月史书·学院数据已部署上线**（R-043，/history 学院数据 tab 线上可用。⚠️ 遗留：线上埋点采集从 8/21 12:00 起完整生效，8/19-8/21 期间只有部分记录（38 条）。服务器 IP 以 `docs/account-passwords.md` 为准：47.96.158.104）
> 所在设备：白机（判定依据：周四 10:49 起工作日白天时段）
