...
> 最后更新：2026-08-20 16:35（白机本轮：**男德通多模态一期·图片理解**（主Agent glm-5.2 + 视觉子Agent doubao-seed-2-0-mini-260428 标准端点，计划模式获批后实施，全链路 Playwright 实测通过：上传→SSE视觉识别事件→回答基于图片→images列落库+描述回写；未部署。部署时 prod.db 需 ALTER TABLE chat_turns ADD COLUMN images TEXT + pm2 restart + 确认 VOLC_VISION_MODEL 环境变量。同日上午产出 Qwen3.8-27B 调研：视频/图片输出系误传，落档 00-调研/，commit e81e331））
...
2. uploadChatImage 误用 `res.json(success({url}))`（success 第一参是 res），上传成功但响应 500
