...
> 最后更新：2026-08-23 18:05（黑机：**男德通检索管线四项工程化优化（token 节约+检索效率）**。院长问「额度消耗快」，审计确认主因是黑机当天验证测试（3 次全量分析触发 429，18:55 重置），线上服务无异常消耗路径；顺势落地四项优化：①P0-1 检索数据跨 Agent 按 id 去重（person/mentioned 上下文重叠，实测省 33%）；②P0-2 `idx_group_messages_nickname_msgTime` 联合索引+幂等脚本 `server/scripts/add-nickname-index.js`（nickname 查询 61ms→3ms，**服务器部署时需执行**）；③P1-3 planner 结果缓存（归一化 key 10min TTL/200 条 LRU，同问法零规划成本）；④P1-4 topic 检索结果缓存（同关键词 10min 复用，零查询零 LLM）。commit `a507943` 已推送。**遗留：前端 dev 浏览器实测（18:55 额度恢复后）、服务器执行索引脚本**）
> 所在设备：黑机（判定依据：周日 18:05 周末全天时段）
