...
> 最后更新：2026-08-17 16:40（白机本轮：v3.4.0 发版完成已 push `1556f04`，服务器部署因 SSH 认证受阻待通道恢复；R-042 调研落档待裁决）
> 所在设备：白机（判定依据：周一 14:01 工作日白天时段）
> 稳定版本：线上 **v3.3.0**（门户趣味化+晚自习深色+移动端适配）；**v3.4.0 星河问已发版待部署**（远端 `1556f04`，SSH 认证受阻）
