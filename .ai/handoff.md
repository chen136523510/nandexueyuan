...
> 最后更新：2026-07-24（白机发版 v2.1.0 部署 R-003 + 文档同步）
> 所在设备：白机（荣耀笔记本）
> 稳定版本：`b040608`（生产环境，已部署，线上 v2.1.0）
> **当前阶段**：R-003 已发版 v2.1.0 上线；需求池 R-009 战斗系统已登记，待黑机世界观创作
...
- [ ] **R-009 德塔战斗系统**（需求池已登记）：调研方案见 `prd/01-需求文档/04-德塔/01-需求/德塔战斗系统调研方案.md` V2
  - **下一步**：黑机世界观创作（基于文档第1.3节骨架）→ 世界观产出后白机提炼游戏性 → PRD MECE → ADR-005
  - **调研核心**：泰拉瑞亚风格（鼠标朝向+左键攻击），4种怪物行为（追击/远程/范围/召唤），塔楼=安全区/塔外=战斗区

- [x] **R-007 发版流程已验证**：v2.1.0 发版已完整走通 release-helper 流程（版本号 bump + 公告 + 部署）
