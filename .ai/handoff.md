...
> 最后更新：2026-08-23 22:35（黑机：**本轮收尾+部署阻塞确认**。R-049 全量分析管线 + BUG-74/75 修复 + 检索四项优化（去重/索引/缓存）均已在 master 提交推送，**但未部署到正式环境**——黑机宽带 22 端口被运营商限制，SSH 连不上服务器（`47.96.158.104:22` Connection timed out，非 IP/密钥问题）。部署必须项：①服务器执行 `cd server && node scripts/add-nickname-index.js`（nickname 联合索引，本地实测 61ms→3ms）；②前端 `npm run build`（需先 `npm install` 补 `@dagrejs/dagre`，否则岁月史书模块 build 失败）。当前线上仍是 v3.6.0，不受影响。**遗留：白机明天 SSH 后执行 deploy.sh + 索引脚本**）
> 所在设备：黑机（判定依据：周日 22:35 周末全天时段）
> 稳定版本：**v3.6.0 线上**（男德通 AI 优化：glm-5.3 + FTS5 v2 + 记忆压缩 + 人设重构 + 多轮追问）
