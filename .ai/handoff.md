...
1. **火山引擎抠图API替换jimp方案** - 院长找到语义级抠图API，定价0.0014元/张，Key通用。调研文档已落档 `prd/01-需求文档/00-调研/火山引擎图像背景移除API.md`。黑机实施：确认Access Key -> 写调用脚本 -> 重抠7套立绘 -> 替换正式资产 -> 退役jimp脚本
2. **其他角色立绘** - 序章已出见/幸/添3角色立绘+表情差分；其他角色（睿/荣/丘/杰/汪神/沐阳）立绘待按出场顺序出图
3. **rembg语义抠图替换** - ~~当前jimp floodfill+空洞填补方案可用~~（已被火山API方案替代，本项废弃）。若火山API不可用时 rembg 可作备选
