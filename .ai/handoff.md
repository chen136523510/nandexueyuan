...
> 最后更新：2026-07-27（白机：R-009 战斗系统阶段1 全链路实现 + 本地联调通过）
> 所在设备：白机（荣耀便携本）-> **待黑机接手**
> 稳定版本：`b040608`（生产环境，已部署，线上 v2.1.0）
> **当前阶段**：R-009 战斗系统阶段1 代码完成 + 本地验证通过，**未部署**（代码在工作树，未 commit）

---

## 当前状态：战斗系统阶段1 已实现并本地验证通过

**生产环境未变动，仍为 `b040608`（线上 v2.1.0）。本轮代码全部在本地工作树，未 commit。**
...
- [ ] **R-009 战斗系统阶段1 收尾**（代码已完成 + 本地验证通过，待院长决定）：
  - 选择 commit 方式：直接 push master / 切 feature 分支 / 先人工试玩
  - 是否部署：院长明确指示后才部署（AGENTS.md 部署纪律）
  - **人工试玩重点**：鼠标攻击手感、怪物追击节奏、HP 条显示、死亡复活过渡

### 黑机接手（战斗系统阶段2 / 美术迭代）

- [ ] **R-009 战斗系统阶段2**（装备系统 + 怪物扩展，调研方案第8节）：
  - 材料合成基础装备（数值+机制差异化）
  - 装备穿戴/卸下/叠加渲染
  - 精英怪物 + 成品装备掉落
  - 裂隙封印交互 + 奖励（含 Inventory 表）
  - 远程/范围/召唤型怪物（schema behaviorType 字段已预留）
  - 投射物系统（远程型需要）
- [ ] **R-003 精灵表/立绘继续优化**（可选，当前已入库可用）
  - 生成脚本：`scripts/gen_walk_sprites_api.py` -> `scripts/batch_cutout_walk.py` -> `scripts/assemble_walk_spritesheet.py`
  - set5 side 一致性偏差待修正（Canny ControlNet -> img2img 重绘）
