...
> 最后更新：2026-08-04 14:47（白机：命名规范+恪/汐改名+编剧5条裁决+宁/饶新角色+饶暗线设定+班回填+hallmark入库）
> 所在设备：白机（荣耀便携本）
> 稳定版本：v2.4.0 已部署上线（commit `811410d`）；最新文档 commit `07d9265`（饶暗线+正太诅咒+学院动机，已推送）
> **当前阶段**：M-G1 引擎核心 PoC ✅ + 序章四幕全量落地 ✅ + 存档系统 ✅ + Seedream调研 ✅ + 10角色脸模入库 ✅ + 序章6张背景图接入 ✅ + 见/幸/添立绘8张+表情差分 ✅ + 立绘舞台状态机制 ✅ + 立绘rembg重抠8/8 ✅ + 立绘位置规则(小腿中部对齐) ✅ + 序章结束场景热点交互 ✅ + 部署上线 ✅ + 存档系统优化(自动恢复+新建从头) ✅ + 热点添对话Q&A问答+主线隔离 ✅ + 世界格局地图生成+地图面板交互 ✅ + 序章旁白优化(四幕44->42句)+pro_302穿帮修正 ✅ + 图片预加载机制(首次加载+进度条) ✅ + 世界书评审+补充11条目 ✅ + 设定集v1.4过时内容修正 ✅ + 第一章大纲产出 ✅ + 社区站视觉体系统一 ✅ + 第一章编剧评审(5条改进建议待裁决) ✅ + 美术资产清理(旧版归档+rembg同步) ✅ + 班形象设计定稿+脸模入库 ✅ + image-gen SKILL生成分层流程补充 ✅ + NPC命名调子规范(六大势力+禁音译名) ✅ + 第一章编剧5条建议院长裁决落地 ✅ + 新角色宁(学院潜伏者)+饶(虚空教团二号) ✅ + 饶暗线设定(国会事变操盘/阿瓦隆交手汪神/汪神正太诅咒) ✅ + 班设定回填(世界书5.10定稿) ✅ + hallmark设计技能入库 ✅

> ⚠️ **网络环境**：GitHub SSH 22 端口当前被限（超时），已配置 `~/.ssh/config` 让 `github.com` 自动走 `ssh.github.com:443` 备用通道。`git push/pull` 正常使用 `git@github.com:...` 即可，SSH 会自动路由。若黑机也遇 22 超时，检查该 config 是否存在。

---

## 白机本轮产出（2026-08-04 14:47）

### NPC命名规范 + 编剧5条裁决 + 新角色宁/饶 + 饶暗线 + 班回填（本轮）

**1. NPC命名调子规范（世界书铁律15）**
- 六大势力命名调子表：学院=单字绰号/帝国=单字庄重古意/共和国=单字务实/部落=单字粗犷/海盗=绰号水海江湖/草原=双字田园/虚空教团=单字本名+宗教名号
- 禁止音译名：科夫->恪（恪尽职守）、西娜->汐（晚潮，姐姐潮随汪神远征失踪）
- 全局替换4个文件（世界书/编剧产出/第一章剧情/handoff）

**2. 第一章编剧5条改进建议院长裁决落地**
- ①第二幕加东北商路异常铺垫（添有情报网，宁信息第三幕才到）
- ②第三幕商人改为三张照片直接给正确信息（删"铁鸟"猜测链，黑机生成P2）
- ③第二幕幸帝桥跨幕回响4分支差异化台词（A令牌="身份很好用吧"/A无令牌="帝国只欢迎同路人"/B="秩序不讲人情"/C="你似乎挺讨厌麻烦的"）
- ④村民删掉"屏障"台词（村民不知海上情况）
- ⑤C选项=最优解，⑥第二幕幸对话待优化 -> 均⚠️标记待黑机处理

**3. 新角色宁（世界书5.13）**
- 第一批漂泊者，潜伏民主共和国任财务大臣（身份只塔楼核心知道）
- 开朗活泼不怒自威，身材高大壮硕相貌潇洒
- 第一章第三幕共和国事变照片来源链路：宁->商会->商人->添（玩家不知宁存在）
- 丘可能已觉察蛛丝马迹（伏笔）

**4. 新角色饶（世界书5.14）+ 暗线设定**
- 虚空教团二号"饶先知/饶先导"，死灵法术，从毁灭中汲取力量
- 明面信虚空实际不信，只借力量不争权。扎小辫子，消瘦灵动
- **国会事变幕后操盘手**：让各方彼此消耗（共和国vs教团/帝国vs草原/帝国vs学院/海盗国动荡）=他的养料
- 与汪神在阿瓦隆交手，**饶负伤养伤**（不亲自上阵，幕后谋划），**汪神中陷阱+诅咒变正太(12岁)**（还政难题）
- 第一章第三幕可疑信使确定为饶的探子（挑拨帝国与学院，玩家不知）
- 揭晓时机：解救汪神后彻底揭晓，未解救则慢慢揭晓
- 铁律14伏笔保护新增5条

**5. 学院深层动机补充**
- 见担心虚空裂隙最终蔓延到地球（第三位面）-> 学院拼命净化裂隙的根本动力
- 世界是否和平是当地人的选择，学院不干涉；玩家是否净化/介入完全自由

**6. 班设定回填**
- 世界书5.10从"形象待补"->完整角色卡（身份/视觉/性格/说话风格/关键关系）
- 速查手册NPC表新增班

**7. hallmark设计技能入库**
- `.zcode/skills/hallmark/`（106文件）提交推送，供双机共用

**8. 方位修正**
- 看地图确认：帝国=西北/共和国=东北/塔楼=西南/海盗=南方远海
- 指共和国用"东北"不用"北边"（北边=帝桥/山河走廊方向）

### git 提交推送
- 10个commit：aaf4c8a(hallmark) / 2e3874a(班回填) / e9a0ae1(命名规范) / 992eb07(5条裁决) / f8f500e+00c8e96(方位修正) / 886ff60(宁+饶) / 2dd1655(饶暗线) / 07d9265(饶负伤+正太+学院动机)
- 另有 sync-docs 提交 `131ec7b`（需求池+handoff同步）
- **其他窗口产出**（`f7106e0`，非本会话）：视频生成API调研（字节Seedance+8家厂商对比），已合入 master
- 全部已推送 GitHub

### 待办（交接黑机）

**第一章（优先）**：
- [ ] ⚠️ 第二幕幸对话整体优化（院长指示，黑机专门处理）
- [ ] ⚠️ 选择③ C选项=最优解，具体分支情节（院长指示，黑机专门处理）
- [ ] 院长裁决编剧5条建议已落地 -> 逐幕打磨台词 -> 产出第一章完整剧本
- [ ] 恪（帝国军官）/汐（海盗头子）形象设计（第一章另两个新 NPC）

**班立绘（黑机出图）**：
- [ ] 班日常装「送葬人」基准立绘 full + 表情差分 -> 基于已入库脸模
- [ ] 班立绘 rembg 抠图 -> 入库 public/visualnovel/portraits/ban/

**美术需求**：
- [ ] 第三幕三张共和国事变画像（飞机撞大楼/丘拦第二架/丘当选，草原辗转传抄风，P2）
