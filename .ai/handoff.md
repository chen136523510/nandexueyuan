...
> 最后更新：2026-08-21 13:40（白机：**男德通 AI 优化两批全部落地**。第二批院长裁决 13 项痛点（commit `6717042`）：FTS5 方案A（unicode61+预分词，2字词可命中，7/7验证通过）/ 同义词缓存（LRU 200组 TTL 1h）/ parseTasks 兜底 / 黑机超时 15s->60s / 温度常量 TEMPS 集中 / 人名 personStat/personMessages 改 IN 精确匹配（mentioned 保持 LIKE）/ 圈外常谈人物（开开/周姐待院长确认身份）/ 记忆压缩（>20turns 自动摘要+前端提示条）/ 多轮追问（lastIntent 注入规划）/ prompt caching 确认火山自动生效 / 弃用 Agent 标注工程经验保留 / dev.db 过时标注 / PRD 重写 v2 + 需求池 R-045/046/047 登记。第一批 commit `2a1304a`：人设重构+glm-5.3+闲聊上下文感知+版本号动态化。全量审计文档 commit `2af467e`。**两批均未部署**（部署纪律），院长过目后随下次发版上线。⚠️ 部署时需：① 服务器 .env VOLC_MODEL 改 glm-5.3 ② prod.db 执行 `ALTER TABLE chat_sessions ADD COLUMN summary TEXT` ③ 服务器跑 `node scripts/rebuildFtsV2.js` 重建 FTS5 v2 索引。服务器 IP 以 `docs/account-passwords.md` 为准：47.96.158.104）
> 所在设备：白机（判定依据：周五 11:14 起工作日白天时段）
> 稳定版本：**v3.5.0 线上**（男德通多模态一期·图片理解：发图提问+视觉识别+思考过程可见）
