...
> 最后更新：2026-08-21 15:20（白机：**v3.6.0 发版 + 公告两处修复已上线**。① v3.6.0 发版三件套（package.json/seedVersion 公告/CHANGELOG，commit `b953a8b`），公告按院长原话 4 条（glm-5.3/二字词汇检索/10轮压缩/上下文管理），已灌入线上 ② ⚠️ 事故修复：seedVersion 幂等策略从"已存在则 update"改为"已存在则跳过"（commit `52e1331`）——院长线上手动编辑的 v3.5.0 公告内容曾被 seedVersion 覆盖，改为保护手动编辑后重新跑 seedVersion 验证跳过生效 ③ 公告弹窗标题栏常驻：VersionHistoryDialog modal-card 改 flex column，header flex-shrink:0 固定，body 独立滚动（院长反馈"滚动时标题和关闭按钮要常态展示在最上方"），线上实测滚动到底 header 位置纹丝不动。**线上验证**：公告 v3.6.0 显示正常 / 弹窗滚动实测 headerFixed=true / API 前端均 200。本轮 AI 优化部署见上轮记录（FTS5 v2 30 秒重建+考研命中 202 块+端到端 20.6s）。服务器 IP 以 `docs/account-passwords.md` 为准：47.96.158.104）
