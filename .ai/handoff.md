...
## 黑机本轮产出（2026-08-06 23:20）

### 美术批量产出（6 张图入库 + 第二幕叙事修正）

**入库清单**：
1. 见·场景B端杯立绘 `public/visualnovel/portraits/dean/cup.png`（serious 定版差分）
2. 帝国军装设计稿 `美术资产/帝国军装/imperial_uniform_v1_01.png`（二战德军参照，深灰蓝+金色，正面+背面，无人物纯服装）
3. 恪·基准立绘 `public/visualnovel/portraits/ke/normal.png`（35岁/板寸/深灰眼/帝国军装，rembg 抠图入库）
4. 草原村口背景 `public/visualnovel/bg/village_entrance.png`（替换 village_gate CSS 占位 ×7）
5. 回程草原路背景 `public/visualnovel/bg/grassland_road.png`（替换 grassland_morning 复用 ×2）
6. 幸二次造访场景图 `public/visualnovel/bg/ch2_xing_arrival.png`（基于大厅生成，幸进门+禁卫剪影）

**代码改动**：
- chapter1.js：恪立绘接入哨卡段（right 位）+ 草原村口/回程背景替换 + **第二幕叙事流程修正**（v1 造访图→v2~v6 大厅立绘→v7 谈判全景，院长反馈"谈判图过早放出"）
- BackgroundLayer.vue：REAL_BG_MAP 新增 village_entrance / grassland_road / ch2_xing_arrival

**文档同步**：
- 美术资产 README：恪立绘/军装/6张背景图清单更新
- 世界书 5.11：恪条目回填形象设计（35岁/板寸/深灰眼/帝国军装）
