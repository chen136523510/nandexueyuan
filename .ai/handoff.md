...
> 最后更新：2026-08-22 12:18（黑机：**BUG-72 黑机WS检索通道修复（wss://）+ BUG-73 手机端男德通输入框遮挡修复**。① 院长反馈男德通出现"黑机离线降级模式"提示，排查为 8/8 SSL 上线后 80 端口 301 强制跳转踢断 `ws://` Worker（ws 客户端不跟随 301），改 `.env` 用 `wss://` 后认证成功，已用 PM2 守护启动 search-worker（注意：**该 worker 没配开机自启，重启电脑后需手动 `npx pm2 start src/searchWorker.js --name search-worker`**），commit `7820f49`。② 院长手机反馈男德通输入框被底栏遮挡，根因是 ChatView scoped CSS `:global(body.has-bottom-nav) .chat-page` 混搭写法被 Vue 编译器静默丢弃，规则移到全局 `src/styles/base.css`，线上注入式实测输入框与底栏零重叠，**改动已推送但需重新构建部署才在线上生效**（本地 `npm run build` 被 `@dagrejs/dagre` 缺失挡住，岁月史书模块遗留，下机需 `npm install` 补装），commit `97047a4`）
> 所在设备：黑机（判定依据：周六 12:18 周末全天时段）
