...
> 最新提交：待推送（含三层塔楼改造 + 6 个 bug 修复 + P2 NPC AI 对话全栈）
> **当前阶段**：
> - P5 美术：立绘 + 像素精灵 + 场景瓦片 + 三层塔楼改造 **全部完成**
> - P2 NPC AI 对话：全栈 6 阶段完成（待 API 额度重置后联调）
...
- [ ] **P2 联调测试**：API 额度重置后（用户说过两天）测试真实 AI 回复
- [ ] **部署到生产**：本轮改动较多（P5 + P2 + 塔楼），待用户确认后执行 `bash deploy.sh`
- [ ] 德塔 P4：角色创建系统（优先级：中）
...
5. **塔楼爬梯手感**：冷却逻辑已加，F12 日志可能还有少量重复（不影响功能），如手感差再调
6. **门开门视觉**：当前用半透明模拟开门，无专用"打开的门"贴图，后续可补
