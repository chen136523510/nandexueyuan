...
> 最后更新：2026-07-27（黑机：ADR-005 世界观承载方式决策 + 战斗系统计划摸排 + 性能调研）
> 所在设备：黑机（RTX 4070 主力机）→ **待白机接手**
> 稳定版本：`b040608`（生产环境，已部署，线上 v2.1.0）
> **当前阶段**：R-009 战斗系统阶段1 计划已摸排，代码 0 行起步；世界观承载方式 ADR-005 已锁定

---

## 当前状态：方向性决策已锁，战斗系统阶段1 待开工

**生产环境已部署并验证通过：`b040608`（线上版本 v2.1.0），未变动**

### 黑机本轮工作总结（2026-07-27，commit `7547722` + 修正提交）

本会话是**纯调研 + 方向性决策**，无代码改动，文档已全部同步推送 master：

1. **战斗系统阶段1 代码现状摸排**：10 步计划对照代码，全部待新建（`shared/`、`game-server/src/schema/`、`game/objects/` 均无战斗文件）；数值已核对设定集 v1.3 第 9.3 节，可直接采用
2. **服务器性能调研**：2核2G 跑战斗系统（20人规模）绰绰有余，**否决黑机代劳战斗 tick**（破坏实时性），详见下方「白机已验证」
3. **ADR-005 世界观承载方式决策**：CG/漫画演出层 + 联机状态同步 + 否决切 3D
4. **AI 视频/漫画产品调研**：Vidu 首选（已确认多主体一致性）、腾讯混元次选，即梦/可灵/万相待白机或院长手核定价
...
- [ ] **R-009 德塔战斗系统阶段1**（计划已摸排，代码 0 行起步）
  - **计划文档**：上一份《德塔战斗系统·阶段1 实现方案（含多人同步）》10 步计划（混合权威：玩家移动客户端权威，怪物/HP/攻击判定服务端权威）
  - **数值权威**：设定集 v1.3 第 9.3 节（普通怪 hp50/atk6/def3/res3，与战斗数值设计文档一致，直接采用）
  - **代码现状**：`shared/`、`game-server/src/schema/`、`game/objects/` 均无战斗相关文件，全部待新建
  - **遗留坑已解决**：角色坐标系（origin/offset）handoff 此前记录的问题已确认解决，可放心做怪物碰撞判定
  - **执行前置确认**（待院长拍板）：①怪物美术=纯色块还是tile拼贴 ②阶段1范围=只做追击型还是搭4种行为框架

- [x] **ADR-005 德塔世界观承载方式已锁定**（2026-07-27）
  - **决策**：剧情承载=CG/漫画演出层（P0 纯漫画 ComfyUI+IP-Adapter，P1 可选视频 Vidu）；所有进度类状态联机同步（禁止单机 localStorage）；**否决切 Unity 3D**
  - **对战斗系统影响**：无直接影响，演出层是独立后续工作。本决策提前锁定承载方式，避免战斗做完剧情无处安放
  - **核心约束**：项目初衷是"小集体抽象娱乐"，不可过度工程化
  - 详细见 `prd/01-需求文档/00-调研/decisions/ADR-005-德塔世界观承载方式.md`
...
|------|-----|
| Git 分支 | `master` |
| 最新 commit | `b040608`（已推送 + 已部署，线上版本 v2.1.0） |
