...
> 最后更新：2026-08-20 17:20（白机本轮：**v3.5.0 发版部署上线 男德通多模态一期·图片理解**（主Agent glm-5.2 + 视觉子Agent doubao-seed-2-0-mini-260428 标准端点，计划模式获批后实施，全链路 Playwright 实测通过：上传→SSE视觉识别事件→回答基于图片→images列落库+描述回写。功能 commit `82d2e33` + gitignore `b7ce794` + 文档 `ec1a500`。部署含 prod.db ALTER TABLE+uploads/chat 目录+公告 v3.5.0。同日上午产出 Qwen3.8-27B 调研：视频/图片输出系误传，落档 00-调研/，commit e81e331））
