...
> **当前阶段**：v2.1.1 已部署，德塔形态重构战略规划已锁定方向（饥荒式），待启动 M-1 氛围层
...
- **方向**：横版像素风 -> 饥荒式展示形态（等距俯视角 + 环境氛围叙事 + 手绘暗黑童话美术风）
- **决策依据**：ADR-005 预留的 2D 俯视角升级路径
- **7个里程碑**：M-1 氛围层 → M-2 等距PoC(决策门) → M-3 ADR-006+美术PoC(决策门) → M-4 引擎重构 → M-5 美术迁移 → M-6 叙事接入 → M-7 上线v3.0.0
...
- [ ] ⛰️ downhill **M-1 环境氛围层**（昼夜/天气/光影/音效，纯增益，可立即启动）
- [ ] ⛰️ uphill **M-2 等距俯视角 PoC**（关键决策门，失败则回退横版）
- [ ] ⛰️ uphill **M-3 ADR-006 + 美术风格 PoC**（关键决策门，依赖 M-2 通过）
- [ ] ⛰️ downhill **M-4 等距引擎重构**（依赖 M-2+M-3 通过）
- [ ] ⛰️ downhill **M-5 手绘美术资产迁移**（黑机产能，可与 M-4 并行）
- [ ] ⛰️ downhill **M-6 世界观叙事接入**（CG演出层，依赖 M-4）
- [ ] ⛰️ downhill **M-7 正式上线 v3.0.0**

### 并行任务（非重构路线图，持续进行）

- [ ] ⛰️ downhill **R-009 战斗系统阶段2**（装备系统 + 怪物扩展，与 M-4 等距重构时适配）
