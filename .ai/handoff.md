...
> 最后更新：2026-08-23 17:46（黑机：**男德通全量数据分析 map-reduce 管线落地（R-049）+ BUG-74/75 修复**。① 新建 `server/src/agents/fullAnalysisAgent.js`：范围圈定（复用各 agent 条件：人物/话题 FTS5 v2+旧版降级+LIKE 兜底/提及/时间）→ 一次性取全量 `id+LENGTH(content)`（0.2s/9.4万行）→ 内存按 24k 字符/批切分 → 上限 40 批超限等距抽样并明示 → map 并发 4 批查询感知摘要（带用户问题/重试退避）→ 分层 reduce（12 份/组合一）→ 底稿 ≤15k 字符注入现有分析流；考研话题真调 LLM 实测：3029 条全量→5 批→底稿 3882 字符（人物观点演变+金句+时间线），86s。② `orchestrator.js` 全量意图路由：planner prompt 加 `full:true` 规则+示例、matchQuickPattern 正则兜底（`全量|完整分析|逐条|全部消息|每一条`）双保险、parseTasks 透传、dispatchAgent 分流本地执行不走黑机（避开 60s 超时）。③ BUG-74：personMessages/mentioned 的 `limit ?? 30` 把黑机 `null` 全量吃掉近一月（`??` 只放行 undefined），改显式三分支。④ BUG-75（存量）：topicSearchAgent 两处 catch 引用 try 内 `const ftsQuery`，FTS5 报错时 catch 自身 ReferenceError 吞掉真实错误，本地缺 v2 表环境必现。⑤ SQL 括号：OR 条件与外部 AND 组合统一括号包裹（dry-run 抓到 BETWEEN 返回 93873 条而非 642）。⑥ `ChatView.vue`：📚 全量分析分组+mapping/reducing phase+「第 x/y 批」进度徽章。**⚠️ 验证中途火山 coding plan 5h 额度耗尽（429 AccountQuotaExceeded，当日 18:55 重置），前端浏览器实测待额度恢复后补做；代码已完成未部署**。需求池 R-049 已登记）
> 所在设备：黑机（判定依据：周日 17:46 周末全天时段）
