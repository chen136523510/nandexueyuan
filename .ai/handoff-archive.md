...
> 本文件是 handoff.md 的历史产出归档（2026-09-15 handoff 瘦身时迁出，内容原样保留未删）。
> 当前战术状态请读 `.ai/handoff.md`；历史里程碑查根 CHANGELOG.md / pm/需求池.md「已完成」表。
> 想知道某次改动的详细记录：先查这里对应的日期节，再查各模块 changelog。

---

## 上一轮产出摘要（白机 2026-09-20 14:08·v3.8.0 已上线；2026-09-23 17:39 白机迁入归档）

- **P0 火山 coding 订阅 2026-09-20 到期致男德通主链路断 → v3.8.0 切 DeepSeek 官方已部署上线**（commit `5ae6aa0`）：llm.js 双通道（DEEPSEEK_API_KEY 走官方/否则回退火山）+ deepseek-flash 文本识图同模型 + env 单文件化。线上 .env 已写 DEEPSEEK 三行（VOLC 保留作回退），服务器探针 5/5（识图 1.6s 准确），火山到期危机解除。后续观察 DeepSeek 按量计费余额；清空线上 DEEPSEEK_API_KEY 即可秒回退
- 其他：R-057 星河问扩建登记需求池；00-调研四类归类；调试截图专区规则

## 上一轮产出摘要（白机 2026-09-20 18:22·纯调研轮；2026-09-23 17:39 白机迁入归档）

- **丘序明 admin 排查闭环**：四路核实"操作从未落库"（非显示 bug），院长网页重操作后线上 role=admin 14:55 生效
- **R-058 电子宠物登记+调研落档**（commit `48ed042`/`e6003be`）：三路并行（Q宠/拓麻/旅蛙考古、MemGPT/星野记忆架构、VPet/Shimeji 互动）→《电子宠物游戏性与AI设计调研》；技术方案三轮初版共识（伪 Live2D 起步等，后被 2026-09-23 五轮探讨部分推翻）
...
- **产出**：`prd/01-需求文档/00-调研/03-产品与游戏设计/群聊数据挖掘产品调研.md`——6 类 12 选题（编年史/峰值日考古/群友图鉴/夜猫子榜/考研 Chronicle/游戏兴衰/复读殿堂/梗百科/互动网络/对线名场面/反直觉洞察榜等）+ 生产管线（SQL 零失真 + LLM 人工审稿双闸 + 静态 JSON 代际化）+ 展示三方案（推荐独立静态页起步 + 年度报告 H5 收尾）+ 内容红线五条（群外真人脱敏/隐私不出静态页/政治降级/人物卡逐人审）+ 三期分期
...
- **调研文档**：`prd/01-需求文档/00-调研/02-工具与AI服务/可视化剧情编辑器选型调研.md`（三路结论合并统一版；Agent B 自落的《剧情对话编辑器调研.md》已删除避免重复）
...
- **调研落档**：`prd/01-需求文档/00-调研/03-产品与游戏设计/抽象小剧情设计与B站亚文化叙事调研.md`
...
- **调研落档**：`prd/01-需求文档/00-调研/03-产品与游戏设计/AI NPC游戏形态与德塔改造方向调研.md`（commit `bd50a63`）
...
- **调研落档** `prd/01-需求文档/00-调研/04-美术/LOGO设计与美术理念调研.md`：设计原则（Paul Rand/Haviv/Airey）、七大类型学、大学校徽传统（外圆内盾+环排+格言）、游戏/社区案例（TGideas/星穹铁道变体/埃菲尔塔形解法）、favicon 规范
...
- 落档 `prd/01-需求文档/00-调研/01-技术/RAG检索策略与工程化调研.md`（commit `37d8053`）
...
- `prd/01-需求文档/00-调研/01-技术/提示词工程调研-图片与视频生成.md`（tag堆叠已过时/语义模型提示词最佳实践/各模型要点/Seedream专项/视频提示词要素清单）
- `prd/01-需求文档/00-调研/02-工具与AI服务/MiniMax视频生成API使用指南.md`（官方txt去重整理为md，9章节）
- image-gen SKILL 更新：画风描述改中文语义、负面词改可选、补视频提示词指导

#### 3. 视频分镜与导演艺术调研
- `prd/01-需求文档/00-调研/04-美术/视频分镜与导演艺术调研.md`（分镜格式/运镜技术/动作场景设计/30秒叙事节奏/AI视频分镜实践/各模型对比）
...
#### 9. 动作设计与武术指导调研
- `prd/01-需求文档/00-调研/04-美术/动作设计与武术指导调研.md`（漫威动作方法论/弓箭手近战弓术7层/魔法师施法三段式/打斗三幕结构/动作经济原则/喘息空间/AI视频动作描述六大规则）
- video-director 技能更新：角色动作签名（弓箭手7层+魔法师6层）、打斗三幕、30秒节拍分配

#### 10. 凡人修仙传战斗设计调研
- `prd/01-需求文档/00-调研/03-产品与游戏设计/凡人修仙传战斗设计调研.md`（四段式层次：试探->升级->转折->决胜/心理博弈/藏底牌/韩立谨慎战术流/修仙元素转译表）
...
>
> 📄 完整调研文档：`prd/01-需求文档/00-调研/01-技术/GUI自动化测试与前端可访问性调研.md`（按知识库调研归档规范五段式结构：背景/重点调研对象/横向对比/总结与建议/待确认事项）
...
- 产出文档：`prd/01-需求文档/00-调研/02-工具与AI服务/gemini-nano-banana-image-generation.md`
