...
- **变更人**：陈梓键（黑机）
- **背景**：院长发现男德通对话出现「⚠️ 当前高性能计算节点（黑机）离线，本次查询使用降级模式」。排查：`searchWorker.js` 实际在跑但每 5 秒重连循环，报 `Unexpected server response: 301`
- **变更内容**（均为配置/文档，无代码改动）：
  1. `server/.env`：`CLOUD_WS_URL` 由 `ws://www.nandexueyuan.top/search-hub` 改为 `wss://...`（.env 不入库，其他跑 Worker 的机器需手动同步）
  2. `prd/01-需求文档/04-德塔/deploy-production.md`：三处生产地址 `ws://` → `wss://`（§3.2 配置示例、Q2 方案 A、Q4 切换说明、预期日志），部署检查清单下追加 BUG-72 警示块
- **验证**：wss 直连测试发送 auth 包收到 `{"type":"auth_ok"}`；`node src/searchWorker.js` 实跑输出「连接已建立，发送握手... / 认证成功，黑机已上线」
- **教训**：见 bug-log BUG-72——SSL 基础设施变更必须回头审查所有明文端点配置，浏览器流量正常 ≠ 全部流量正常
- **状态**：✅ 已修复（worker 重启后恢复全量检索）
...
- **变更人**：陈梓键（黑机）
- **背景**：院长线上实测反馈"广州游玩当灯泡事件细说"回答的日期错误（说成 7月11号，实际 2026-01-01 元旦）。日志显示分析阶段 prompt 长达 184,082 字符
- **变更内容**：`server/src/agents/timeSearchAgent.js` 话题块摘要按月聚合（chunksByMonth），每月只取前 8 个代表性话题块、keywords 截 100 字、标题标注每月总块数
- **验证**：服务器 node 直跑，2026 全年查询 prompt 184,082 -> 7,278 字符，1~8 月按月分布+每月话题清晰
- **状态**：✅ 已部署上线（`8cc3f13`）
- **关联文档**：bug-log.md BUG-67 / server/src/agents/changelog.md

---

### [feat] 门户趣味化（首页学院大门 + 大厅动效 + 晚自习深色主题 + 移动端底部导航）

- **时间**：2026-08-13 14:17 ~ 15:38
- **变更人**：陈梓键（白机）
...
- **验证**：`npm run build` 通过 ✅；生产部署 deploy.sh 9 步全通过 ✅；线上验证（HTTPS 200 / 新构建生效 / 版本公告 v3.3.0 / API+游戏服务器 PM2 online / 日志无报错）✅；浏览器人工体验待院长过目
- **部署修复**：部署期间发现并修复 BUG-65（prod.db 缺 spaceState 列）与 BUG-66（colyseus 0.16.25 坏发布），详见 bug-log.md
- **文件**：`src/views/HomeView.vue`、`src/views/MainView.vue`、`src/views/AdminView.vue`、`src/views/ProfileView.vue`、`src/views/LoginView.vue`、`src/views/RegisterView.vue`、`src/views/FeedbackView.vue`、`src/views/ChatView.vue`、`src/views/WallView.vue`、`src/components/TopBar.vue`、`src/components/ThemeToggle.vue`（新增）、`src/components/BottomNav.vue`（新增）、`src/components/WordCloud.vue`、`src/components/UserAvatar.vue`、`src/components/ProfileDialog.vue`、`src/composables/useTheme.js`（新增）、`src/styles/variables.css`、`src/styles/base.css`、`src/App.vue`、`src/router/index.js`、`game-server/package.json`
- **状态**：✅ 已部署上线 v3.3.0（服务器 commit `1f1068c`，公告已灌入）
...
- **变更人**：陈梓键（黑机）
- **背景**：v3.2.0 男德通 AI 升级后续，完成知识库四层升级部署 + 时间范围检索新功能 + 多个 bug 修复 + 数据库纯净重建上线
- **变更内容**：
  1. **数据库部署上线**：538,915 条（排除私聊和其他群，仅西德+东德）+ 5,372 分块 + FTS5 索引。踩坑：服务器 `DATABASE_URL=file:./prod.db`，scp 上传 dev.db 后需 `cp dev.db prod.db` 覆盖 Prisma 实际读取的文件
  2. **dbInfoAgent 发言排行修复**：查 Top 30 -> JS 层 `resolveName` 合并同一人不同昵称 -> 重新排序取 Top 10。不排除 `nickname='我'`（=陈梓键 11 万条）。修复后 Top 1 陈梓键 114,723 条
  3. **knowledge.js 昵称映射补全**：`做题体孝子（暂时）`->丘序明 / `MICO`->陈梓键
  4. **新增 timeSearchAgent**：时间范围检索 Agent，LLM 把自然语言时间转 `startDate/endDate`，按 `chunkDate BETWEEN` 查数据库。按日/月聚合统计 + 话题块摘要 + 每天抽样消息。解决"7月份聊了什么"不再搜字符串
  5. **v3.2.1 发版**：patch 版本，公告灌入线上
- **验证**：线上 538,915 条 / 陈梓键 114,723 第一 / 2026 全年 148,026 条按月分布完整 / v3.2.1 公告显示
- **状态**：✅ 已部署上线 v3.2.1
- **关联文档**：根 CHANGELOG.md / server/src/agents/changelog.md
...
- **时间**：2026-08-09
- **变更人**：陈梓键（白机）
- **背景**：院长要求用真人照片（陈睿）为睿制作新脸模，并基于新脸模生成基准立绘
- **变更内容**：
  1. **脸模 v2**：`美术资产/睿/face_v2_01.png`（2K，磁盘保存 gitignore）。真人合影照片（锁五官，`public/man/ChenRui/`）+ 旧脸模 face_v1（锁画风/服饰）双参考图，Seedream Pro 生成。院长定版（含细金属框圆形眼镜）
  2. **基准立绘**：`美术资产/睿/full_v1_01.png`（磁盘保存）+ 运行时 `public/visualnovel/portraits/rui/normal.webp`（832×1216，146KB）。以脸模单张参考图生成，rembg 抠图 + WebP q95 压缩。白袍执政者形象（象牙白长袍+烟灰腰封+银链黑曜石+暗银戒指+深灰靴）
  3. **美术资产 README 更新**：脸模清单睿行更新（v1->v2 定版，补眼镜特征）；立绘清单新增睿行
- **文件**：`public/visualnovel/portraits/rui/normal.webp`（新增）、`美术资产/README.md`（更新）
- **状态**：待提交

---

### [文档] 美术设计规范V2.1(画风改中文语义) + 国会事变剧本初稿 + CG视频提示词精写

- **时间**：2026-08-09
- **变更人**：陈梓键（白机）
- **背景**：①出图/视频工具已从 ComfyUI+LoRA 切换到 Seedream/MiniMax 语义模型，旧英文关键词画风描述不适用；②院长要求国会事变剧情剧本；③院长要 CG 视频提示词
- **变更内容**：
  1. **美术设计规范 V2 -> V2.1**：§1 画风定义从英文关键词（`thick oil painting, impasto brushstrokes...`）+ ComfyUI/LoRA 管线改为完整中文语义描述（适配语义模型）；§2.2 发色速查表英文词改中文；§2.3 年龄控制英文词改中文；§7 设计原则去掉 LoRA 引用
  2. **国会事变剧本初稿**：`02-设计/剧情设计/国会事变-剧本.md`，完整版8场（黎明前->国会大厅->结果宣布->撞击->第二架->废墟誓言->就任->尾声箭羽）+ 精简版一段话（撞击->拦截->演讲->当选）+ 饶暗线附录。丘身份=游侠/平民（非议员）
  3. **CG 视频提示词精写**：15秒版本，事无巨细描述（建筑结构/飞行器细节/人物穿着动作/箭与晶石/光影变化/音效），搭配 `qiu/rogue.webp` 参考图锁画风。院长已用 MiniMax H3 生成首个 PoC（`美术资产/丘/hailuo-assets/asset-542758497214140425.mp4`，已 gitignore）
  4. **.gitignore 补 mp4 忽略**：美术资产目录下 `*.mp4` 不入库
- **文件**：`02-设计/美术设计/美术设计规范.md`（V2.1）、`02-设计/剧情设计/国会事变-剧本.md`（新增）、`.gitignore`（补 mp4）
- **状态**：美术设计规范+gitignore 未提交；剧本已提交（`1b73905`）
- **关联文档**：需求池 R-033（CG视频）
...
- **时间**：2026-08-07
- **变更人**：陈梓键（白机）
- **背景**：①院长提出空间概念需求（R-035：塔楼/草原/大陆三级布局，自由探索不写死代码），先技术打穿；②第三幕台词过稿完成需落地；③预加载与玩家进度错位（有存档玩家预热错位）
- **变更内容**：
  1. **空间机制引擎**（R-035，架构级）：`explore` 节点属性（dialogue/end 均可带）→ 探索态；合成节点渲染（渲染层与剧情模式共用）；LOCATIONS 地点图（bg/hotspots/exits/onEnter/unlockedBy）；store 空间状态（currentLocation/currentExploreLocation/unlockedLocations/visitedLocations）+ enterExplore/travelTo；HotspotLayer 支持出口（travel action）+ 出口样式；NdeVisualNovelView 探索态 desc 横幅
  2. **存档接口扩展**：GameSave 新增 spaceState JSON 字段（前后端 + 迁移文件 `20260807120000_add_game_save_space_state`，旧存档向后兼容：无字段自动进探索态）
  3. **第二幕结尾迁移探索态**：自由活动（hall_free）/走廊看月亮（corridor_free）/回房间（room）三地点 + 班/睡觉路由分流（ch2_done 变量）；修正自由活动热点坐标（见右添左）
  4. **第三幕「东来的信」全量落地**（86+ 节点）：商队段（三张事变**照片**）/段六讨论+判断选择（benefit_choice）/过渡段探索态（见 Q&A 4 问/添 Q&A 4 问含照片查看占位/班 4 句定稿）/信使段（房间探索态→走廊马蹄声→下楼选择→扔信（丘的信 qiu_letter）→死灵气息讨论→17 按 contract 分支）
  5. **舞台设计修正**：第二幕收尾场景切换（幸离场后才恢复立绘与大厅）、段六收束表情切换（serious→calm）、探索态 Q&A 说话人角色色
  6. **预加载优化**：按存档章节预加载 + 切章增量预加载 + 修复预加载 404（只收集 REAL_BG_MAP 真实图，抽 bgMaps 公共模块）——首页静默预加载曾尝试后撤回（拖卡网站，见下方条目）
  7. **部署修复**：deploy.sh 迁移漏检（BUG-58，grep 模式匹配实际输出）
- **验证**：引擎层 474 节点全量遍历 0 错误；浏览器 dev+线上完整剧情（序章→第一章→第二幕探索态→第三幕信使段→章节结束）全通过；存档/读档/旧存档兼容通过；线上部署 v3.0.0 完成
- **状态**：已部署上线（v3.0.0，master `af65726`）
- **关联文档**：`04-技术方案/空间与地图机制技术方案.md`、`演出设计-第一章-第三幕.md`、需求池 R-035

---

### [feat] 预加载优化：按存档章节预加载 + 切章增量预加载（首页静默预加载已撤回）

- **时间**：2026-08-07
- **变更人**：陈梓键（白机）
- **背景**：①原 preloadAssets 写死预加载序章，有存档玩家（接续玩第一章）预热错位——序章图白下、第一章图没预热（院长反馈"每次进来都要载入那么久"）；②曾尝试"首页后台静默预加载"（MainView onMounted），但**几十 MB 图片在首页后台下载拖卡网站**（院长反馈"网站卡的要死"），已撤回
- **变更内容**：
  1. **按存档章节预加载**（initGame 重构）：先读自动存档确定玩家章节 → 只预加载该章节（有存档→接着玩的章节；无→序章）+ 进度条，替代原来写死的序章预加载
  2. **切章增量预加载**（unlockChapter）：跳转新章节时 fire-and-forget 补预加载新章节图片
  3. **预加载 404 修复**：collectAssetUrls 只收集 REAL_BG_MAP 真实图（bg/black 等 CSS 占位 key 跳过），抽公共模块 `data/bgMaps.js`（BackgroundLayer + store 共用）
  4. ~~首页静默预加载~~（MainView onMounted + preloadChapterImages）——**已撤回**（后台下载拖卡网站，commit `a663113`）
- **最终方案**：回到"进德塔时按进度加载 + 进度条"（玩家进德塔时加载，期间可最小化去做别的工作）；首页零游戏图片下载
- **验证**：build 通过；线上验证首页 0 图片下载 + 进德塔缓存命中秒进；首次无缓存时进度条正常
- **相关**：图片体积优化（背景 2-8MB/张）留待黑机出图时处理（无损压缩或高画质 WebP）——治本项
...
- **时间**：2026-08-06
- **变更人**：陈梓键（白机）
- **背景**：院长指出第二幕"只有台词没有演出设计"（立绘差分/场景/调度不完整），且项目缺少演出设计输出规范——建立规范并以第二幕实践校验
- **变更内容**：
  1. **《演出设计输出规范》**（新文档，v1.1）：每幕 5 件套（🎭 立绘表情表/舞台调度/场景清单/内联标注/美术需求表）+ 输出流程 + 红线；v1.1 补 3 缺口（分支内表情节奏/[道具]语法/CG 复用背景判定）
  2. **《演出设计-第一章-第二幕》**（新文档，5 件套实践）：幸三差分设计、帝桥回响四句两键复用映射、B 树表情节奏、羊皮卷道具演出
  3. **谈判段方案乙（院长 08-06 拍板）**：三张谈判全景图（对峙/幸看玩家/见看玩家，圆桌+四角色入画+羊皮卷发光）替代立绘演出，幸差分降级 P2 可选；走廊夜复用 CG-1
- **状态**：已按此落地第二幕代码（f1f619a）
- **关联文档**：演出设计输出规范.md、演出设计-第一章-第二幕.md
