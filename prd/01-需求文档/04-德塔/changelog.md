...
- **变更内容**：
  ...
  6. **预加载优化**：首页（MainView）后台静默预加载 + 按存档章节预加载 + 切章增量预加载；修复预加载 404（只收集 REAL_BG_MAP 真实图，抽 bgMaps 公共模块）
...
### [feat] 预加载优化：首页后台静默预加载 + 按存档章节预加载 + 切章增量预加载

- **时间**：2026-08-07
- **变更人**：陈梓键（白机）
- **背景**：①原 preloadAssets 写死预加载序章，有存档玩家（接续玩第一章）预热错位——序章图白下、第一章图没预热（院长反馈"每次进来都要载入那么久"）；②图片总量大（50-100MB），首次进入等待久
- **变更内容**：
  1. **首页后台静默预加载**（HomeView onMounted）：登录用户浏览首页时后台 new Image() 悄悄下载玩家接下来要玩的章节图片（无进度条、不阻塞），进德塔时命中 HTTP 缓存秒进
  2. **按存档章节预加载**（initGame 重构）：先读自动存档确定玩家章节 → 只预加载该章节（有存档→接着玩的章节；无→序章），替代原来写死的序章预加载
  3. **切章增量预加载**（unlockChapter）：跳转新章节时 fire-and-forget 补预加载新章节图片
  4. 抽取 `collectAssetUrls` 公共函数（背景/立绘/地图/空间地点背景），`preloadChapterImages`（静默版）复用
- **验证**：build 通过；浏览器实测 preloadChapterImages 触发（preloaded 防重复 + 58 张图片下载 + 按存档章节序章）；HomeView onMounted 在 dev 缓存下有旧实例假象（BUG-56 同款），生产构建无此问题
- **相关**：图片体积优化（背景 2-8MB/张）留待黑机出图时处理（无损压缩或高画质 WebP）
