...
- **发现时间**：2026-08-17 16:45（v3.4.0 发版部署阶段）
- **环境**：白机 → 生产服务器
- **现象**：`ssh root@47.96.158.95` 报 `Permission denied (publickey...)`，连续重试（显式 -i id_rsa、显式 -p 22）均被拒，一度判定「密钥不在本机/需要专用 deploy key」准备求助院长手动部署
- **根因**：**纯 IP 记忆错误**。正确 IP `47.96.158.104` 一直记录在 `docs/account-passwords.md`「三、生产环境」节，`~/.ssh/id_rsa` 密钥完全有效。凭记忆使用生产资源地址，出错后错误现象（认证被拒）与真实原因（连错机器）完全脱节，排查方向被系统性误导
- **修复**：院长提示「部署相关文件没有说吗？」→ 检索 `docs/` 目录命中 `account-passwords.md` → 用文档 IP 直连一次成功 → 部署 9 步全绿 + 线上 e2e 4/4
- **文件**（规则固化，非代码）：`AGENTS.md`（部署纪律节新增「动手前必查文档」红线）、`.zcode/skills/release-helper/SKILL.md`（新增 Step 7 部署前置检查）、`.ai/handoff.md`（网络环境节升级为部署铁律警示）
- **教训**：凡涉及服务器地址/SSH 账号密钥/数据库连接串/环境路径的操作，**动手前强制检索 `docs/` 与 handoff，以文档为准，禁止凭记忆**。凭记忆操作生产资源 = 高概率出错，且错误现象会误导排查方向（连接类故障优先怀疑地址/端口/目标，而不是先怀疑凭证）。「查询部署文档」固化为发版/部署流程第一步强制动作。
...
- **发现时间**：2026-08-13 16:30（院长线上实测反馈）
- **环境**：生产环境（男德通 time_search 大范围时间查询）
- **现象**：问"广州游玩当灯泡事件细说"，AI 回答"2026年7月11号"，实际该事件在 2026-01-01 元旦话题块；日志显示分析阶段 prompt 总长度 184,082 字符
- **根因**：timeSearchAgent 大范围查询（"2026年全年"含 1,466 个话题块）时，把全部块的 keywords+summary 逐块拼接塞给 LLM，18 万字符远超 LLM 有效处理能力，不同话题块的时间信息全部混乱，LLM 还把对话历史里的"2022/7/11"（数据库最早日期）混入回答
- **修复**：话题块摘要按月聚合（chunksByMonth），每月只取前 8 个代表性话题块、keywords 截 100 字、标题标注每月总块数。2026 全年查询 prompt 从 184,082 降到 7,278 字符
- **文件**：`server/src/agents/timeSearchAgent.js`
- **状态**：已修复并部署上线（`8cc3f13`），待院长线上复测确认
- **教训**：子 Agent 返回给分析阶段 LLM 的数据必须有硬性体积上限。聚合统计（按月/按周）优于全量拼接；大范围查询应压缩为"统计+代表性样本"，不能把检索结果原样全塞
...
### BUG-50：序章文案 8 处与设定集/时间线冲突

- **发现时间**：2026-08-03（白机通读序章全部游戏文案）
- **环境**：`src/visualnovel/data/scripts/` 四幕 script 文件（文案层）
- **现象**（8 处）：
  1. 幸自称「礼部主事」与设定集（帝国外交部部长）及添 Q&A 口径冲突
  2. pro_brief_2「三年前向陛下臣服+设军事哨站」与附庸时间线冲突（应为停贡不停名，哨站是闪电战后才建），且与幸自己 pro_ask_city_1「名义附庸」自相矛盾
  3. pro_117 见用「她」指代玩家（玩家性别未定）
  4. pro_qa_next_4「回头让院长给你储物戒指」但第三幕已发纳戒
  5. pro_qa_world_2「十年前政变」（实为 18 年前，A.V.100）
  6. pro_qa_world_5「汪神两年前去南极」（远征 A.V.108，且学院不知汪神失踪）
  7. pro_qa_world_6「油气多」（设定集为矿产香料）
  8. pro_qa_home_2「添罕见地低迷」未按旁白优化终审（应删"罕见地"）
- **根因**：① 台词创作时未对照设定集（幸官职笔误、政变年份、油气）；② 功能开发时序（储物系统）先于台词打磨，台词未同步；③ 旁白优化终审回填时遗漏 qa_home_2 一处
- **修复**：按院长逐条定稿修改 8 处 text（详见 changelog 2026-08-03 记录），只改文案不动逻辑
- **教训**：改台词前必须对照设定集最新版；功能/剧情双轨推进时，旧台词要在剧情落地后复查一遍时序；旁白优化终审结果要逐条勾对回填，防止遗漏
- **状态**：✅ 已修复（未 commit，待院长确认后提交）
...
### BUG-47：dean/calm + dean/gentle 带背景原图丢失（无法重抠）

- **现象**：rembg 批量重抠立绘时，发现 dean/calm 和 dean/gentle 两张表情的**带背景原图在黑机本地找不到**
- **根因**：黑机用 Seedream「脸模+服装双参考」生成这两张表情时，直接抠图入库（`public/visualnovel/portraits/dean/`），**未保存带背景的过程图到 seedream-test**。`.ai/seedream-test/院长/` 只有 v1_01(古风失败)/v1_02(水墨失败)/v1_03(成功=serious) 三张，均非 calm/gentle
...
### BUG-48：Playwright 视觉模型对暗色调游戏内立绘识别失败

- **现象**：浏览器验收立绘时，DOM 确认 `<img>` 已渲染（490x1242, opacity:1, active 状态），但 Playwright 截图的视觉识别反复报告"只有破碎天空背景"，看不到立绘
- **根因**：游戏内背景为暗紫色调（虚空降临/塔楼夜景），院长立绘也是深色黑袍，视觉模型（图像识别）无法区分人物与暗背景，统称为"破碎天空"。**截图本身没问题**（像素分析确认立绘区域有 173 个红色边框标记像素）
- **修复**：用 PIL 裁剪立绘区域 + 放大 2 倍 + 提亮 1.8 倍 + 增强对比度 1.3 倍后，视觉模型成功识别出人物
- **教训**：暗色调游戏画面验收不能用视觉模型直接判断，需配合 ①DOM 查询（img.complete + naturalWidth）②像素采样分析 ③裁剪提亮后再看
- **状态**：经验记录

---

## 2026-07-30（白机 序章文案优化+缓几天分支）

---

### BUG-46：缓几天路径第四幕开头死循环（幸已走却让回复幸）

- **现象**：选第三项「缓几天」后跟添聊完、回复幸、幸离去，进入第三幕→第四幕开头时，又跳回添的结束语「下楼去给人家答复吧」，但幸早已走，形成死循环
- **根因**：`pro_cond_met_tian`（第四幕开头判断）用 `met_tian=true` 区分「是否已聊过添」。缓几天路径设了 met_tian=true，该 condition 的 true 分支跳到 `pro_qa_end_1`（添说"下楼回复幸"）。但缓几天流程回复完幸、幸走后回流到这里时，met_tian 仍为 true，于是又走了一遍"回复幸"，而此时幸已离开
- **修复**：`pro_cond_met_tian` 的 `met_tian=true` 分支从 `pro_qa_end_1` 改为 `pro_end`。语义：缓几天路径已聊过添（Q&A 提前完成），到达第四幕时直接序章结束；正常 A/B 路径 met_tian 未设，走 else 进 pro_303 正常聊添
- **验证**：evaluate 模拟三条路径（正常A/B、缓几天、正常聊完添）打印断点确认跳转正确
- **文件**：`prologue.js`（pro_cond_met_tian 节点）
- **教训**：同一变量（met_tian）在不同 condition 里被复用、但隐含语义不同时，要核对每条回流路径是否仍满足该语义。condition 分支修改后，必须遍历所有能到达它的路径做断点验证，不能只推理
- **状态**：已修复

---

### BUG-45：第四幕新增问答id冲突覆盖话题六

- **现象**：院长在 scripts 第四幕新增话题「你有想过回去吗」，但用了 `pro_qa_next_1~4`，和话题六「我接下来该干嘛」的id完全相同
- **根因**：mergeScript 按 id 合并文案，同 id 后者覆盖前者。话题六的 pro_qa_next_1~4 被新问答覆盖，导致话题六文案全部丢失
- **修复**：新问答id改为 `pro_qa_home_1~4`，prologue.js 同步插入节点链 + pro_qa_choice 新增第7选项
- **文件**：`scripts/序章-第四幕-自由探索.script.js`、`prologue.js`
- **教训**：scripts 新增文案时，id 前缀不能和已有节点冲突。mergeScript 是按id覆盖式合并，同id静默覆盖无告警
- **状态**：已修复

---

### BUG-44：pro_refuse_2重复id导致文案丢失

- **现象**：scripts 第二幕 pro_refuse_2 重复出现两次（院长手动编辑文案时新增了一行），导致 mergeScript 后只保留后者（"还有，我希望你能对陛下保持尊重"），丢掉前者整段话
- **根因**：同一 scripts 文件内存在重复 id，mergeScript 按 id 后者覆盖前者
- **修复**：合并为一条完整文案
- **文件**：`scripts/序章-第二幕-法刺来访.script.js`
- **教训**：同 BUG-45，mergeScript 对重复 id 无告警，编辑文案后应检查id唯一性
- **状态**：已修复
