...
### 2026-07-24 HUD 重构决策（战斗系统阶段）

> 触发：战斗系统引入后，原 HUD 三栏布局不足以承载装备/技能/背包信息
>
> **已实现**（R-010 交付，2026-07-24）
...
**变更后**（五栏，已实现）：
```
[角色信息 180px] [技能+装备槽 160px] [背包 160px] [聊天 flex:1] [小地图 146px]
```

**各模块实际实现**：

**模块1 - 角色信息**（左侧）：
- 头像（点击不再用于切换形象，改为**打开角色面板弹窗（立绘+装备+背包+属性）** ✅ 已实现
- 昵称 ✅
- HP 条（战斗系统接入后实时变化） ✅ 已有占位
- MP 条（战斗系统接入后实时变化） ✅ 已有占位
- buff/debuff 图标区 ✅ 已有占位

**模块2 - 技能槽 + 装备槽**（新增，160px，已实现 ✅）：
- 共 8 个格子，当前够用，以后加技能位和更多装备系统时再扩展格子数
- 布局：2行×4列 → 第一行 **4 格装备槽**（头盔/胸甲/护腿/靴子），第二行 **4 格技能槽**（当前用 3 个，预留 1 个） ✅
- 快捷键绑定：技能槽可绑定数字键 1/2/3/4 快速施放 → R-013 实现

**模块3 - 背包**：
- 8 格背包（沿用现有设计） ✅
- 可拖拽到技能槽/装备槽进行穿戴 → R-012 实现数据接入后提供

**模块4 - 聊天**：沿用现有设计 ✅

**模块5 - 小地图**：沿用现有设计 ✅

**头像点击弹窗（博德之门风格角色面板，已实现 ✅）**：
```
┌───────────────────────────────────────────────┐
│  [大型立绘]    [装备栏]         [背包内容]      │
│                武器 □                          │
...
│                靴子 □          物品...        │
│  [属性面板]                                    │
│  ATK 10  DEF 5  RES 3                         │ （基础值按剧情设定 DEF=5/RES=3）
└───────────────────────────────────────────────┘
```
- 立绘左侧大图展示当前形象 ✅
- 装备栏中间，5格（武器 + 4件防具），点击高亮可卸下 ✅ 占位UI完成
- 背包右侧，8格（与底部HUD UI对齐） ✅ 占位UI完成
- 属性面板显示当前基础属性（后续 R-012 接入装备加成） ✅

**形象切换入口迁移**（已按用户需求调整 ✅）：
- 原：点击 HUD 头像 -> 弹出 5 套形象切换器
- 新：头像点击 -> 角色面板弹窗（立绘 + 装备 + 背包）
- 形象切换移至：TopBar 下拉菜单 →「德塔相关设置」弹窗 ✅
  - 弹窗结构：左侧完整立绘 + 右侧5套形象，每套带小头像预览 + 完整行走精灵图展示 ✅
  - 复用 `NdeSettingsDialog.vue` 独立组件，功能拆分清晰 ✅