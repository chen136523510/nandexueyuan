...
> ✅ **2026-08-21 院长裁决：方案A，已实施落地**（tokenizer.js + rebuildFtsV2.js + 查询侧切换，7/7 验证通过）。
>
> **关于"重建索引跑什么"的澄清**（2026-08-21 院长问）：`rebuildFtsV2.js` 是**纯本地 SQLite + JS CPU 操作，不调 LLM、不花钱、不需要黑机**。它做的事：读现有 53.8 万条消息 + 5,372 个分块（LLM 分块总结早已完成，不用重跑），用 tokenizer.js 在 JS 层做中文分词（整词/bigram 切分），把分词结果 INSERT 进新建的 FTS5 v2 虚拟表。batch 500 批量插入，53 万条预计几分钟~十几分钟跑完。与 R-048 向量检索（需调 embedding API 花钱）完全不同。
...
> 院长 2026-08-21 裁决结果已回写到各痛点方案。下表反映裁决后状态。

| 痛点 | 裁决 | 实施状态 |
|------|------|----------|
| 6 glm-5.3 | 接受现状（已切 5.3） | ✅ 本次完成 |
| 15 版本号修复 | 按方案 | ✅ 本次完成 |
| 2 同义词缓存 | 按方案 | ✅ 本次完成 |
| 17 弃用 Agent 标注 | 不删除，标注已弃用+工程经验保留 | ✅ 本次完成 |
| 4 parseTasks 兜底 | 按方案 | ✅ 本次完成 |
| 5 FTS5 日志增强 | 按方案 | ✅ 本次完成 |
| 8 黑机超时 | 提升至 60s（院长裁决从 30s 加码） | ✅ 本次完成 |
| 19 温度常量集中 | 按方案（TEMPS 导出） | ✅ 本次完成 |
| 1 FTS5 方案A | 按方案A（unicode61+预分词），7/7 验证通过 | ✅ 本次完成 |
| 13 人名精确匹配 | personStat/personMessages 改 IN，**mentioned 保持 LIKE**（职责就是搜"别人提到"） | ✅ 本次完成 |
| 16 圈外常谈人物 | knowledge 加 frequentPersons（开开/周姐=群友高中同班同学，院长已确认） | ✅ 本次完成 |
| 22 多轮追问 | 按方案（lastIntent 注入规划） | ✅ 本次完成 |
| 11 prompt caching | 实测：火山自动缓存（cached_tokens=1024），无需代码改动 | ✅ 确认自动生效 |
| 21 记忆压缩 | 超 20 turns 压缩摘要 + 前端提示条 | ✅ 本次完成 |
| 18 旧 PRD | 重写为 v2 + 需求池登记 | ✅ 本次完成 |
| 19 dev.db 过时标注 | 标注已过时（不重命名，文档说明） | ✅ 本次完成 |
| 6/7 延迟 | 接受现状 | - |
| 9 prompt 预算 | 接受现状 | - |
| 10 token 监控 | 接受现状 | - |
| 12 世界书分段 | 接受现状 | - |
| 14 消息抽样 | 接受现状 | - |
| 3 向量检索 | **列入需求池 R-048，后续黑机操作**（分块版 0.75 元+1h，前置实测 embedding 端点） | 📋 R-048 排期 |
| 20 db 命名 | dev.db 标注过时（不重命名） | ✅ 本次完成 |
| 23 出图 | 列入需求池 R-045 | 排期 |
| 24 视频输入 | 列入需求池 R-046，优先级低 | 排期 |
| 25 图片清理 | 暂缓 | - |
| 26 FTS5 增量 | 需慎重调研（R-047），全量分块成本高 | ⛰️ 调研中 |
