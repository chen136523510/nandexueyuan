...
> ⚠️ AutoDL 价格页为 JS 动态加载且登录墙拦截，未能查到实时精确价格。以下为行业经验区间，**非本次查证一手数据**，仅供量级估算。

| 卡型 | 估算时租（CNY） | 来源标注 |
|---|---|---|
| RTX 4090 24GB | ~1.5-2.5 元/时 | 行业经验区间（未查证） |
| RTX 5090 32GB | ~3-5 元/时 | 行业经验区间（未查证） |
| A100 80GB | ~6-10 元/时 | 行业经验区间（未查证） |
| H100 80GB | ~15-25 元/时 | 行业经验区间（未查证） |

> 建议院长如需精确数字，直接登录 AutoDL（autodl.com）控制台查看，或访问恒源云/揽睿星等平台对比。

### 5.1 粗算：自部署 Qwen3.8-27B（4-bit）月成本

- 单卡 4090：2 元/时 × 24h × 30d ≈ **1440 元/月**
- 但按项目实际用量（间歇调用），按需开机更划算：假设日均用 4 小时，约 240 元/月
...
2. **为 LLM + 视觉理解**（男德通升级、朋友圈图片理解）→ Qwen3.8-27B 4-bit 单卡 4090 自部署**技术可行**。按需开机月成本约 240-1440 元。但需评估：现有火山引擎 API 费用是否已高到此数？视觉理解是否有刚需场景？目前 handoff 未记录 LLM API 成本痛点，**建议先量化现有 API 月支出再决定**。
...
- AutoDL 官网（autodl.com）-- 价格页 JS 动态加载+登录墙，未查到实时精确价格（行业经验区间见第五章标注）

### 未交叉验证项

- GPU 租用价格区间（第五章）为行业经验估算，**非本次一手查证**，需院长登录 AutoDL 核实
