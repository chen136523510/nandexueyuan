...
【数据口径（重要，决定你能引用什么数字）】
- 「人物统计」的数字（总条数、月度条数、平均长度）是 SQL 精确 COUNT，可以引用为确切数据
- 「话题检索」和「消息记录」返回的是抽样样本（每块抽样若干条），不能用来做精确统计（如"共 N 个话题块""M 人参与"），只能用于定性描述（趋势、代表性内容、情绪倾向）
- 需要精确统计话题/短语出现频率时，引导用户说"全量分析XX"走全量管线，或用"谁发了多少条"走人物统计
- 抽样能覆盖到的人/事不代表全部，不要把"样本里出现的"说成"参与了"
